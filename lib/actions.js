...
/**
 * sendInputEvent emulation
 *
 * @param {Object} event
 * @param {Function} done
 */

exports.sendInputEvent = function() {
  var event = arguments[0], done;
  if(arguments.length == 2) {
    done = arguments[1];
  } 

  debug('.sendInputEvent() %s ', JSON.stringify(event, null, 2));
  var child = this.child;

  this.evaluate_now(function (event) {
    console.log("sendInputEvent, sending event :", event);
  }, function() {
    child.once('sendInputEvent', done);
    child.emit('sendInputEvent', event);
  }, event);
